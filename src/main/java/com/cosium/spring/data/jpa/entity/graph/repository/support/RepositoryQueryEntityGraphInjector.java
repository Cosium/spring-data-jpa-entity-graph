...
class RepositoryQueryEntityGraphInjector implements MethodInterceptor {
	...
	private static final List<String> EXECUTE_QUERY_METHODS = Arrays.asList("getResultList", "getSingleResult");
	...
	private void addEntityGraphToQuery(Query query) {
		...
		if (!entityGraphCandidate.isValid()) {
			if (entityGraphCandidate.isOptional()) {
				...
				throw new RuntimeException("Cannot apply EntityGraph " + entityGraphCandidate);
			}
		}
